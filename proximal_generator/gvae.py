...
class ContextVAE(nn.Module):
    ...
    def __init__(self, encoder_inner_sizes=[32], decoder_inner_sizes=[32], state_size=9,
        ...
        self.nb_objects = 5
        self.nb_permutations = self.nb_objects * (self.nb_objects - 1)

        dim_mp_input = 18
        dim_mp_output = 3 * dim_mp_input
        self.mp_network = GnnMessagePassing(dim_mp_input, dim_mp_output)

        encoder_layer_sizes = [self.nb_objects + 3 + dim_mp_output] + encoder_inner_sizes
        decoder_layer_sizes = [latent_size + self.nb_objects + 3] + decoder_inner_sizes + [12] # 12 = 4 * 3
    ...
    def forward(self, initial_c, initial_s, current_c):
        ...
        edge_features = self.mp_network(inp_mp)
        ...
        inp_decoder = torch.cat([nodes[:, 0, :], z], dim=1)

        recon_x = self.decoder(inp_decoder)

        res = torch.clone(initial_c)

        res[:, [0, 1, 2, 3, 10, 11, 12, 13, 14, 18, 22, 26]] = recon_x

        return res, means, log_var, z
    ...
    def inference(self, initial_s, initial_c, n=1):
        ...
        # inp_decoder = torch.stack([torch.cat([nodes[:, i, :], z[i]], dim=1)
        #                            for i in range(self.nb_objects)])
        ids = [[0, 1, 2, 3, 10, 11, 12, 13, 14, 18, 22, 26], [0, 4, 5, 6, 10, 14, 15, 16, 17, 19, 23, 27],
