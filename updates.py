...
def up_deep_context(model, policy_optim, critic_optim, context_optim, alpha, log_alpha, target_entropy, alpha_optim, obs_norm, g_desc_norm, anchor_g,
    ...
    # sync_grads(model.single_phi_actor)
    # sync_grads(model.rho_actor)
    sync_grads(model.actor_gnn.message_passing_module)
    sync_grads(model.actor_gnn.node_aggr_module)
    sync_grads(model.actor_gnn.graph_aggr_module)
    ...
    critic_optim.zero_grad()
    qf_loss.backward()
    sync_grads(model.critic_gnn.message_passing_module)
    sync_grads(model.critic_gnn.node_aggr_module)
    sync_grads(model.critic_gnn.graph_aggr_module)
