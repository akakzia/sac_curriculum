...
def up_deep_context(model, policy_optim, critic_optim, context_optim, alpha, log_alpha, target_entropy, alpha_optim, obs_norm, g_desc_norm,
                    atomic_ids, anchor_g, obs_next_norm, g_desc_next_norm, actions, rewards, args):
    # Tensorize
    obs_norm_tensor = torch.tensor(obs_norm, dtype=torch.float32)
    obs_next_norm_tensor = torch.tensor(obs_next_norm, dtype=torch.float32)
    g_desc_norm_tensor = torch.tensor(g_desc_norm, dtype=torch.float32)
    atomic_ids_tensor = torch.tensor(atomic_ids).long()
    ...
    with torch.no_grad():
        model.forward_pass(obs_next_norm_tensor, g_desc_next_norm_tensor, atomic_ids_tensor, anchor_g_tensor)
        actions_next, log_pi_next = model.pi_tensor, model.log_prob
        qf1_next_target, qf2_next_target = model.target_q1_pi_tensor, model.target_q2_pi_tensor
        min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha * log_pi_next
        next_q_value = r_tensor + args.gamma * min_qf_next_target
    ...
    # the q loss
    qf1, qf2 = model.forward_pass(obs_norm_tensor, g_desc_norm_tensor, atomic_ids_tensor, anchor_g_tensor, actions=actions_tensor)
    qf1_loss = F.mse_loss(qf1, next_q_value)
    qf2_loss = F.mse_loss(qf2, next_q_value)
    qf_loss = qf1_loss + qf2_loss
