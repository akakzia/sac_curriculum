...
class RolloutWorker:
    ...
    def generate_rollout(self, true_eval, biased_init=False, animated=False, language_goal=None):

        episodes = []
        for i in range(len(language_goal)):
            observation = self.env.unwrapped.reset_goal(biased_init=biased_init)
            ...
            ep_obs, ep_ag, ep_ag_bin, ep_actions, ep_success, ep_rewards = [], [], [], [], [], []
            ep_lg_id = []
            ...
            for t in range(self.env_params['max_timesteps']):
                ...
                action = self.policy.act(obs.copy(), no_noise, language_goal=language_goal_ep)
