...
def launch(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        for _ in range(args.n_cycles):
            ...
            # Policy updates
            t_i = time.time()
            if episode_count > EXPLORATION_EPS:
        ...
        t_i = time.time()
        if episode_count > EXPLORATION_EPS and args.curriculum_learning:
        ...
        if args.evaluations:
            ...
            rewards = np.array([policy.her_module.compute_reward_masks(episodes[i]['ag'][-1], eval_goals[i], None) for i in range(len(eval_goals))])
