...
def plot_post_activations(c, a, goal_id=1):
    with open('message_encoder_data.pkl', 'rb') as f:
        data = pkl.load(f)
    data = np.array(data[goal_id]).squeeze(1)
    data_norm = [np.linalg.norm(data[:, i, :], ord=np.inf, axis=-1) for i in range(9)]
    x = np.arange(0, 100)
    fig, ax = plt.subplots(1)
    ax.plot(x, data_norm[c], color='orange')
    ax.plot(x, data_norm[a], color='red')
    # for i in range(3):
    #     axs[i, 0].plot(x, data_norm[i])
    # for i in range(3):
    #     axs[i, 1].plot(x, data_norm[i+3])
    # for i in range(3):
    #     axs[i, 2].plot(x, data_norm[i+6])
    plt.savefig('/home/akakzia/DECSTR/results/edges_features.pdf')
...
if __name__ == '__main__':
    ...
    for PLOT in TO_PLOT:
        ...
        if PLOT == 'edges':
            plot_post_activations(c=2, a=-1, goal_id=1)