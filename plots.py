...
def setup_n_figs(n, xlabels=None, ylabels=None, xlims=None, ylims=None):
    fig, ax = plt.subplots(n, 1, figsize=(10, 10), frameon=False)
    # axs = axs.ravel()
    artists = ()
    ...
    if xlabels[0]:
        xlab = ax.set_xlabel(xlabels[0], fontsize='15')
        artists += (xlab,)
    if ylabels[0]:
        ylab = ax.set_ylabel(ylabels[0], fontsize='15')
        artists += (ylab,)
    if ylims[0]:
        ax.set_ylim(ylims[0])
    if xlims[0]:
...
def plot_c_lp_p_sr(experiment_path, true_buckets=True):
    ...
    for cond in conditions:
        ...
        for run in list_runs:
            ...
            counter = 0
            for i in range(4):
                axs.plot(x_eps, data_run['Eval_SR_{}'.format(i)][x], color=colors[i], marker=MARKERS[i], markersize=MARKERSIZE // 3,
            ...
            #     axs[5].plot(x_eps, data_run['B_{}_p'.format(i)][x], color=colors[i], marker=MARKERS[i], markersize=MARKERSIZE//3, linewidth=LINEWIDTH//2)
                ...
                # except:
                #     pass
            leg = axs.legend(['I{}'.format(i) for i in range(1, 6)],
                             loc='upper center',
