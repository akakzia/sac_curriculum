...
def plot_sr_av(max_len, experiment_path, folder, true_buckets=False):
    ...
    condition_path = experiment_path + folder + '/'
    # condition_path = experiment_path
    list_runs = sorted(os.listdir(condition_path))
...
def get_mean_sr(experiment_path, max_len, max_seeds, conditions=None, labels=None, ref='DECSTR_bis'):
...
if __name__ == '__main__':
    ...
    for PLOT in TO_PLOT:
        ...
        if PLOT == 'MPGNN':
            ...
            plot_sr_av(max_len, experiment_path, 'combinations')
            plot_lp_av(max_len, experiment_path, 'combinations')
