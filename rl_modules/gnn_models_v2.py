...
class GnnCritic(nn.Module):
    def __init__(self, nb_objects, aggregation, readout, dim_body, dim_object, dim_mp_input, dim_mp_output, dim_phi_critic_input,
        ...
        self.edge_scoring_fn_1 = nn.Parameter(torch.Tensor(1, dim_mp_output))
        self.edge_scoring_fn_2 = nn.Parameter(torch.Tensor(1, dim_mp_output))
    ...
    def message_passing(self, obs, ag, g):
        ...
        # concat_output_mp = torch.stack([torch.cat([output_mp[i, :, :], output_mp[j, :, :]], dim=-1) for (i, j) in permutations([0, 1, 2, 3, 4, 5], 2)])

        scores_1 = torch.sum((output_mp * self.edge_scoring_fn_1), dim=-1, keepdim=True)
        scores_2 = torch.sum((output_mp * self.edge_scoring_fn_2), dim=-1, keepdim=True)

        scores_1 = scores_1.transpose(0, 1)
        scores_2 = scores_2.permute(1, 2, 0)

        all_scores = self.leakyReLU(scores_1 + scores_2)
        all_attention_coefficients = self.softmax(all_scores)

        att_output_mp = torch.bmm(all_attention_coefficients, output_mp.transpose(0, 1))

        att_output_mp = att_output_mp.permute(1, 0, 2)

        return att_output_mp


class GnnActor(nn.Module):
    def __init__(self, nb_objects, aggregation, readout, dim_body, dim_object, dim_phi_actor_input, dim_phi_actor_output, dim_rho_actor_input, dim_rho_actor_output):
...
class GnnSemantic:
    def __init__(self, env_params, args):
        ...
        self.critic_target = GnnCritic(self.nb_objects, self.aggregation, self.readout, self.dim_body, self.dim_object, dim_mp_input, dim_mp_output,
                                       dim_phi_critic_input, dim_phi_critic_output, dim_rho_critic_input, dim_rho_critic_output)
        self.actor = GnnActor(self.nb_objects, self.aggregation, self.readout, self.dim_body, self.dim_object, dim_phi_actor_input, dim_phi_actor_output,
