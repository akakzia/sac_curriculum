...
class SACAgent:
    def __init__(self, instructions, args, compute_rew):
        ...
        self.model = DeepSetSAC(instructions, self.env_params, args)
    ...
    def act(self, obs, no_noise, language_goal=None):
        with torch.no_grad():
    ...
    def _update_network(self):
        ...
        o, o_next, ag, ag_next, actions, rewards = transitions['obs'], transitions['obs_next'], transitions['ag'], \
        ...
        critic_1_loss, critic_2_loss, actor_loss, alpha_loss, alpha_tlogs = update_deepsets(self.model, self.policy_optim, self.critic_optim, self.alpha, self.log_alpha,
                                                                                            self.target_entropy, self.alpha_optim, obs_norm, obs_next_norm, actions, rewards, language_goals, self.args)
    ...
    def load(self, model_path, args):
        ...
        o_mean, o_std, phi_a, phi_c, rho_a, rho_c = torch.load(model_path, map_location=lambda storage, loc: storage)
