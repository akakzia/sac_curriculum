...
class DeepSetContext:
    def __init__(self, env_params, args):
        ...
        dim_phi_actor_input = dim_phi_encoder_output + self.dim_body + 9
        ...
        dim_phi_critic_input = dim_phi_encoder_output + self.dim_body + 9 + self.dim_act
    ...
    def policy_forward_pass(self, obs, g_desc, anchor_g=None, no_noise=False):
        ...
        obs_objects = [self.observation.narrow(-1, start=self.dim_object * i + self.dim_body, length=9)
                       for i in range(self.num_blocks)]

        # # Initialize context input
        context_input = torch.empty((self.g_desc.shape[0], self.g_desc.shape[1], 4 + 2*9))
