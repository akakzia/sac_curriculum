...
class GnnCritic(nn.Module):
    def __init__(self, nb_objects, nb_permutations, dim_body, dim_object):
        ...
        self.gnn_attention = GnnAttention(dim_attention_input, 256, dim_attention_output)
    ...
    def forward(self, obs, ag, g, act, attended_ids):
        ...
        inp = torch.stack([torch.cat([act, obs_body, pivot_nodes[:, 0, :], torch.max(output_mp[:2, :, :], dim=0).values], dim=1),
                           torch.cat([act, obs_body, remaining_nodes[:, 0, :], output_mp[-2, :, :]], dim=1),
                           torch.cat([act, obs_body, remaining_nodes[:, 1, :], output_mp[-1, :, :]], dim=1)])
...
class GnnActor(nn.Module):
    ...
    def forward(self, obs, ag, g, attended_ids):
        batch_size = obs.shape[0]
    ...
    def sample(self, obs, ag, g, attended_ids):
        mean, log_std = self.forward(obs, ag, g, attended_ids)
        std = log_std.exp()
...
class GnnSemantic:
    ...
    def policy_forward_pass(self, obs, ag, g, anchor_g, no_noise=False):
        ...
        if not no_noise:
            self.pi_tensor, self.log_prob, _ = self.actor.sample(obs, ag, g, attended_ids)
        else:
            _, self.log_prob, self.pi_tensor = self.actor.sample(obs, ag, g, attended_ids)
    ...
    def forward_pass(self, obs, ag, g, anchor_g, actions=None):
        ...
        if actions is not None:
            self.q1_pi_tensor, self.q2_pi_tensor = self.critic.forward(obs, ag, g, self.pi_tensor, attended_ids)
            return self.critic.forward(obs, ag, g, actions, attended_ids)
        else:
            with torch.no_grad():
                self.target_q1_pi_tensor, self.target_q2_pi_tensor = self.critic_target.forward(obs, ag, g, self.pi_tensor, attended_ids)
